    ...
    <AppShell>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="animate-rise surface-card mb-6 p-6">
            <div className="flex items-center gap-5">
              <ProgressRing value={overall} size={96} tone="primary" sublabel="Done" />
              <div className="min-w-0 flex-1">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">
                  Overall progress
                </p>
                <p className="mt-2 inline-flex items-center gap-2 rounded-2xl bg-success/12 px-3 py-1.5 text-success">
                  <Flame className="size-4" strokeWidth={1.75} />
                  <span className="text-sm font-semibold">{streak} day streak</span>
                </p>
                <p className="mt-2 text-xs leading-snug text-muted-foreground">
                  {done} of {tasks.length || 0} tasks completed
                </p>
              </div>
            </div>
          ...
          {upcomingExams.length > 0 ? (
            <Link to="/exams" className="surface-card lift mb-6 flex items-center gap-3 p-5">
          ...
          <div className="grid grid-cols-2 gap-3.5">
            {cards.map((card, i) => (
              <Link
                ...
                className={cn("surface-card lift relative flex flex-col gap-2 p-5")}
              >
                <span className="flex size-11 items-center justify-center rounded-2xl bg-primary/10 text-primary">
                  <card.icon className="size-5" strokeWidth={1.75} />
                </span>
                <p className="text-sm font-semibold leading-tight tracking-tight">{card.title}</p>
          ...
          <h2 className="mb-3 mt-8 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">
            Today's tasks
          </h2>
          {dueToday.length === 0 && pending.length === 0 ? (
            <EmptyState
              ...
              description="Add a task or let the AI planner build today's schedule around your free hours."
              action={
                <Link to="/planner" className="mt-1 text-sm font-semibold text-primary">
                  Generate my plan →
                </Link>
              }
            />
